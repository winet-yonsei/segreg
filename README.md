...
$$y_i'=a_1 + b_1 x_i, \text{if~} x_i < BR$$

    
$$y_i'= a_2 + b_2 x_i, \text{if~} x_i \geq BR$$
...
## 코드 작동 원리
* 작성한 코드는 가장 큰 차이 제곱값을 가지는 구역을 우선적으로 분할한다.
* 분할은 설정한 최대 분할 구역의 갯수가 되거나 모든 구역에서 차이 제곱값이 설정 값보다 아래가 되면 중지한다.
* 설정값으로 음수를 입력하면 해당 조건은 검사하지 않는다.
